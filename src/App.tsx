...
export default function Auxiron(){
  ...
  var [briefSession,setBriefSession]=useState("asia");
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg0,fontFamily:"'DM Mono','Courier New',monospace",color:C.txt0}} className="auxiron-root">
      ...
      <div className="auxiron-content">
        ...
        {/* ── DAILY BRIEF ── */}
        {tab==="brief"&&<div style={{padding:"12px"}} className="fu">
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:14,fontWeight:700,color:C.txt0,letterSpacing:".06em"}}>DAILY BRIEF</div>
              <div style={{fontSize:10,color:C.txt1,marginTop:1}}>Goldman-style · Live web search · Bank forecasts</div>
            </div>
            <button className="tap" onClick={function(){fetchBrief(briefSession);}} disabled={briefLoading}
              style={{background:briefLoading?C.bg2:"linear-gradient(135deg,#c8a840,#e8c858)",color:briefLoading?C.txt2:"#0c1118",border:"none",borderRadius:8,padding:"7px 14px",fontSize:9,fontWeight:700,display:"flex",alignItems:"center",gap:5}}>
              {briefLoading?[<div key="sp" className="sp" style={{width:10,height:10,border:"2px solid "+C.border2,borderTopColor:C.txt1,borderRadius:"50%"}}></div>,"GENERATING…"]:"◎ GENERATE"}
            </button>
          </div>
          <div style={{display:"flex",gap:6,marginBottom:12}}>
            {[["asia","🌏 ASIA OPEN","8am-12pm SGT"],["ny","🗽 PRE-NY","8pm-10pm SGT"]].map(function(s){
              var active=briefSession===s[0];
              return <button key={s[0]} className="tap" onClick={function(){setBriefSession(s[0]);}}
                style={{flex:1,background:active?"rgba(200,168,64,0.12)":C.bg1,border:active?"1px solid rgba(200,168,64,0.4)":"1px solid "+C.border,borderRadius:9,padding:"9px 8px",textAlign:"center"}}>
                <div style={{fontSize:11,fontWeight:active?600:400,color:active?C.goldL:C.txt1}}>{s[1]}</div>
                <div style={{fontSize:9,color:active?C.gold:C.txt2,marginTop:1}}>{s[2]}</div>
              </button>;
            })}
          </div>
          {!brief&&!briefLoading&&<div style={{textAlign:"center",padding:"40px 20px",background:C.bg1,border:"1px solid "+C.border,borderRadius:12}}>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:28,color:C.txt3,marginBottom:8,opacity:0.25}}>◎</div>
            <div style={{fontSize:12,color:C.txt2,letterSpacing:".08em",marginBottom:4}}>GOLDMAN-STYLE MARKET BRIEF</div>
            <div style={{fontSize:10,color:C.txt3,marginBottom:2}}>Uses live web search for current news</div>
            <div style={{fontSize:10,color:C.txt3}}>Bank forecasts · Technicals · Fundamentals · Economic Calendar</div>
          </div>}
          {briefLoading&&<div style={{background:C.bg1,border:"1px solid "+C.border,borderRadius:12,padding:"30px 20px",textAlign:"center"}}>
            <div className="sp" style={{width:24,height:24,border:"3px solid "+C.border2,borderTopColor:C.gold,borderRadius:"50%",margin:"0 auto 12px"}}></div>
            <div style={{fontSize:12,color:C.txt1,letterSpacing:".08em",marginBottom:4}}>SEARCHING WEB + GENERATING BRIEF</div>
            <div style={{fontSize:10,color:C.txt2}}>Fetching live news · Bank forecasts · Economic calendar</div>
            <div style={{fontSize:10,color:C.txt3,marginTop:4}}>Takes 15-30 seconds</div>
          </div>}
          {brief&&<div className="fu">
            <div style={{background:"linear-gradient(135deg,rgba(200,168,64,0.12),rgba(200,168,64,0.04))",border:"1px solid rgba(200,168,64,0.3)",borderRadius:10,padding:"13px",marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                <span style={{fontSize:10,color:C.gold,letterSpacing:".1em",fontWeight:600}}>TODAY'S HEADLINE</span>
                {brief.marketRisk&&<span style={{fontSize:9,padding:"2px 7px",borderRadius:4,fontWeight:600,
                  background:brief.marketRisk==="ELEVATED"?"rgba(240,64,64,0.15)":"rgba(72,144,248,0.1)",
                  color:brief.marketRisk==="ELEVATED"?C.dn:C.blue,
                  border:"1px solid "+(brief.marketRisk==="ELEVATED"?C.dn:C.blue)+"44"}}>
                  {brief.marketRisk} RISK
                </span>}
              </div>
              <div style={{fontSize:14,fontWeight:600,color:C.txt0,lineHeight:1.5,marginBottom:8}}>{brief.headline}</div>
              {brief.weeklyTheme&&<div style={{fontSize:11,color:C.txt1,lineHeight:1.6,borderTop:"1px solid "+C.border,paddingTop:8}}>{brief.weeklyTheme}</div>}
            </div>
            {brief.overnightSummary&&<div style={{background:C.bg1,border:"1px solid "+C.border,borderRadius:10,padding:"13px",marginBottom:10}}>
              <div style={{fontSize:10,color:C.txt2,letterSpacing:".1em",marginBottom:6}}>🌙 OVERNIGHT SUMMARY</div>
              <div style={{fontSize:13,color:C.txt0,lineHeight:1.75}}>{brief.overnightSummary}</div>
            </div>}
            {brief.institutionalForecasts&&brief.institutionalForecasts.length>0&&<div style={{background:C.bg1,border:"1px solid "+C.border,borderRadius:10,padding:"13px",marginBottom:10}}>
              <div style={{fontSize:11,color:C.txt1,letterSpacing:".1em",marginBottom:8,fontWeight:600}}>🏦 BANK FORECASTS</div>
              <div style={{display:"grid",gap:6}}>
                {brief.institutionalForecasts.map(function(f,i){
                  return <div key={i} style={{background:C.bg2,border:"1px solid "+C.border,borderRadius:8,padding:"10px 12px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                      <div style={{display:"flex",alignItems:"center",gap:7}}>
                        <span style={{fontSize:12,fontWeight:700,color:C.gold}}>{f.bank}</span>
                        <span style={{fontSize:10,color:C.txt2}}>on {f.instrument}</span>
                      </div>
                      <span style={{fontSize:9,color:C.txt3,background:C.bg1,padding:"2px 6px",borderRadius:4,border:"1px solid "+C.border}}>{f.timeframe}</span>
                    </div>
                    <div style={{fontSize:12,fontWeight:500,color:C.txt0,marginBottom:3}}>{f.forecast}</div>
                    <div style={{fontSize:11,color:C.txt2,lineHeight:1.5}}>{f.rationale}</div>
                  </div>;
                })}
              </div>
            </div>}
            {brief.instrumentReports&&brief.instrumentReports.length>0&&<div style={{marginBottom:10}}>
              <div style={{fontSize:11,color:C.txt1,letterSpacing:".1em",marginBottom:8,fontWeight:600}}>📊 INSTRUMENT DEEP DIVE</div>
              <div style={{display:"grid",gap:8}}>
                {brief.instrumentReports.map(function(inst,i){
                  var isUp=inst.priceChange&&inst.priceChange.startsWith("+");
                  var fundClr=inst.fundamental&&inst.fundamental.rating==="BULLISH"?C.up:inst.fundamental&&inst.fundamental.rating==="BEARISH"?C.dn:C.amber;
                  var techClr=inst.technical&&inst.technical.bias==="BULLISH"?C.up:inst.technical&&inst.technical.bias==="BEARISH"?C.dn:C.amber;
                  var rrClr=inst.riskReward==="FAVORABLE"?C.up:inst.riskReward==="UNFAVORABLE"?C.dn:C.amber;
                  return <div key={i} style={{background:C.bg1,border:"1px solid "+C.border,borderRadius:10,padding:"13px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                      <div>
                        <div style={{fontFamily:"'Syne',sans-serif",fontSize:16,fontWeight:700,color:C.txt0}}>{inst.symbol}</div>
                        <div style={{fontSize:10,color:C.txt2,marginTop:1}}>{inst.name}</div>
                      </div>
                      <div style={{textAlign:"right"}}>
                        <div style={{fontSize:14,fontWeight:600,color:C.txt0,fontVariantNumeric:"tabular-nums"}}>{inst.currentPrice}</div>
                        <div style={{fontSize:11,fontWeight:500,color:isUp?C.up:C.dn}}>{inst.priceChange}</div>
                      </div>
                    </div>
                    <div style={{display:"flex",gap:5,marginBottom:10,flexWrap:"wrap"}}>
                      {inst.fundamental&&<span style={{fontSize:9,padding:"3px 8px",borderRadius:4,fontWeight:600,background:"rgba(0,0,0,0.3)",color:fundClr,border:"1px solid "+fundClr+"44"}}>FUND: {inst.fundamental.rating}</span>}
                      {inst.technical&&<span style={{fontSize:9,padding:"3px 8px",borderRadius:4,fontWeight:600,background:"rgba(0,0,0,0.3)",color:techClr,border:"1px solid "+techClr+"44"}}>TECH: {inst.technical.bias}</span>}
                      {inst.riskReward&&<span style={{fontSize:9,padding:"3px 8px",borderRadius:4,fontWeight:600,background:"rgba(0,0,0,0.3)",color:rrClr,border:"1px solid "+rrClr+"44"}}>R/R: {inst.riskReward}</span>}
                      {inst.sentiment&&<span style={{fontSize:9,padding:"3px 8px",borderRadius:4,background:"rgba(72,144,248,0.1)",color:C.blue,border:"1px solid rgba(72,144,248,0.2)"}}>{inst.sentiment.positioning}</span>}
                    </div>
                    {inst.fundamental&&<div style={{background:C.bg2,border:"1px solid "+C.border,borderRadius:8,padding:"9px 11px",marginBottom:6}}>
                      <div style={{fontSize:9,color:fundClr,letterSpacing:".08em",marginBottom:3}}>FUNDAMENTAL — {inst.fundamental.keyDriver}</div>
                      <div style={{fontSize:12,color:C.txt0,lineHeight:1.6}}>{inst.fundamental.analysis}</div>
                    </div>}
                    {inst.technical&&<div style={{background:C.bg2,border:"1px solid "+C.border,borderRadius:8,padding:"9px 11px",marginBottom:6}}>
                      <div style={{fontSize:9,color:techClr,letterSpacing:".08em",marginBottom:4}}>TECHNICAL — {inst.technical.pattern}</div>
                      <div style={{display:"flex",gap:8,marginBottom:5}}>
                        <div style={{flex:1,background:"rgba(40,204,120,0.08)",border:"1px solid rgba(40,204,120,0.2)",borderRadius:5,padding:"5px 8px",textAlign:"center"}}>
                          <div style={{fontSize:8,color:C.up,marginBottom:1}}>SUPPORT</div>
                          <div style={{fontSize:13,fontWeight:600,color:C.up}}>{inst.technical.keySupport}</div>
                        </div>
                        <div style={{flex:1,background:"rgba(240,64,64,0.08)",border:"1px solid rgba(240,64,64,0.2)",borderRadius:5,padding:"5px 8px",textAlign:"center"}}>
                          <div style={{fontSize:8,color:C.dn,marginBottom:1}}>RESISTANCE</div>
                          <div style={{fontSize:13,fontWeight:600,color:C.dn}}>{inst.technical.keyResistance}</div>
                        </div>
                      </div>
                      <div style={{fontSize:12,color:C.txt0,lineHeight:1.6}}>{inst.technical.note}</div>
                    </div>}
                    {inst.sentiment&&<div style={{background:C.bg2,border:"1px solid "+C.border,borderRadius:8,padding:"9px 11px",marginBottom:6}}>
                      <div style={{fontSize:9,color:C.blue,letterSpacing:".08em",marginBottom:4}}>MARKET SENTIMENT</div>
                      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                        {inst.sentiment.institutionalBias&&<span style={{fontSize:10,padding:"3px 8px",borderRadius:4,background:"rgba(0,0,0,0.2)",color:inst.sentiment.institutionalBias==="BULLISH"?C.up:inst.sentiment.institutionalBias==="BEARISH"?C.dn:C.amber,border:"1px solid rgba(255,255,255,0.08)"}}>INST: {inst.sentiment.institutionalBias}</span>}
                      </div>
                      {inst.sentiment.cotNote&&<div style={{fontSize:11,color:C.txt1,marginTop:5,lineHeight:1.5}}>{inst.sentiment.cotNote}</div>}
                    </div>}
                    {inst.bankConsensus&&<div style={{background:"rgba(200,168,64,0.07)",border:"1px solid rgba(200,168,64,0.2)",borderRadius:8,padding:"9px 11px",marginBottom:6}}>
                      <div style={{fontSize:9,color:C.gold,marginBottom:3}}>🏦 BANK CONSENSUS</div>
                      <div style={{fontSize:12,color:C.txt0,lineHeight:1.55}}>{inst.bankConsensus}</div>
                    </div>}
                    {inst.traderNote&&<div style={{background:"rgba(40,204,120,0.06)",border:"1px solid rgba(40,204,120,0.2)",borderRadius:8,padding:"9px 11px"}}>
                      <div style={{fontSize:9,color:C.up,marginBottom:3}}>◈ TRADER NOTE</div>
                      <div style={{fontSize:12,color:C.txt0,lineHeight:1.55}}>{inst.traderNote}</div>
                    </div>}
                  </div>;
                })}
              </div>
            ...
            {/* ── ECONOMIC CALENDAR ── */}
            {brief.economicCalendar&&<div style={{background:C.bg1,border:"1px solid "+C.border,borderRadius:10,padding:"13px",marginBottom:10}}>
              <div style={{fontSize:11,color:C.txt1,letterSpacing:".1em",marginBottom:10,fontWeight:600}}>📅 ECONOMIC CALENDAR</div>
              <div style={{display:"flex",gap:5,marginBottom:10}}>
                {[["week","THIS WEEK"],["month","THIS MONTH"],["quarter","REMAINING QUARTER"]].map(function(t){
                  var active=calTab===t[0];
                  return <button key={t[0]} className="tap" onClick={function(){setCalTab(t[0]);}}
                    style={{flex:1,background:active?"rgba(200,168,64,0.12)":C.bg2,border:active?"1px solid rgba(200,168,64,0.4)":"1px solid "+C.border,
                      color:active?C.goldL:C.txt2,borderRadius:7,padding:"5px 4px",fontSize:8,fontWeight:active?600:400,textAlign:"center"}}>
                    {t[1]}
                  </button>;
                })}
              </div>
              {calTab==="week"&&brief.economicCalendar.thisWeek&&<div style={{display:"grid",gap:5}}>
                {brief.economicCalendar.thisWeek.map(function(ev,i){
                  var impClr=ev.impact==="HIGH"?C.dn:ev.impact==="MEDIUM"?C.amber:C.txt2;
                  var cFlag=ev.country==="US"?"🇺🇸":ev.country==="EUR"?"🇪🇺":ev.country==="JPY"?"🇯🇵":ev.country==="GBP"?"🇬🇧":"🌐";
                  return <div key={i} style={{background:C.bg2,border:"1px solid "+(ev.impact==="HIGH"?"rgba(240,64,64,0.25)":C.border),borderRadius:8,padding:"10px 12px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        <span style={{fontSize:12}}>{cFlag}</span>
                        <span style={{fontSize:12,fontWeight:600,color:C.txt0}}>{ev.event}</span>
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:5}}>
                        <span style={{fontSize:9,fontWeight:700,color:impClr,background:"rgba(0,0,0,0.3)",padding:"2px 6px",borderRadius:4,border:"1px solid "+impClr+"44"}}>{ev.impact}</span>
                      </div>
                    </div>
                    <div style={{display:"flex",gap:10,marginBottom:5}}>
                      <span style={{fontSize:10,color:C.gold}}>{ev.date} {ev.time}</span>
                      {ev.forecast&&<span style={{fontSize:10,color:C.txt2}}>Exp: {ev.forecast}</span>}
                      {ev.previous&&<span style={{fontSize:10,color:C.txt2}}>Prev: {ev.previous}</span>}
                    </div>
                    {ev.tradingImplication&&<div style={{fontSize:11,color:C.txt1,lineHeight:1.5}}>→ {ev.tradingImplication}</div>}
                  </div>;
                })}
              </div>}
              {calTab==="month"&&brief.economicCalendar.thisMonth&&<div style={{display:"grid",gap:5}}>
                {brief.economicCalendar.thisMonth.map(function(ev,i){
                  var impClr=ev.impact==="HIGH"?C.dn:ev.impact==="MEDIUM"?C.amber:C.txt2;
                  var cFlag=ev.country==="US"?"🇺🇸":ev.country==="EUR"?"🇪🇺":ev.country==="JPY"?"🇯🇵":ev.country==="GBP"?"🇬🇧":"🌐";
                  return <div key={i} style={{background:C.bg2,border:"1px solid "+C.border,borderRadius:8,padding:"10px 12px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    ...
                    <div style={{fontSize:10,color:C.gold,marginBottom:4}}>{ev.date}</div>
                    {ev.whyItMatters&&<div style={{fontSize:11,color:C.txt1,lineHeight:1.5}}>→ {ev.whyItMatters}</div>}
                  </div>;
                })}
              </div>}
              {calTab==="quarter"&&brief.economicCalendar.remainingQuarter&&<div style={{display:"grid",gap:5}}>
                {brief.economicCalendar.remainingQuarter.map(function(ev,i){
                  var impClr=ev.impact==="HIGH"?C.dn:ev.impact==="MEDIUM"?C.amber:C.txt2;
                  var cFlag=ev.country==="US"?"🇺🇸":ev.country==="EUR"?"🇪🇺":ev.country==="JPY"?"🇯🇵":ev.country==="GBP"?"🇬🇧":"🌐";
                  return <div key={i} style={{background:C.bg2,border:"1px solid "+C.border,borderRadius:8,padding:"10px 12px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:5}}>
                        <span style={{fontSize:9,color:C.vix,background:"rgba(184,88,240,0.1)",padding:"2px 6px",borderRadius:4,border:"1px solid rgba(184,88,240,0.2)"}}>{ev.month}</span>
                        <span style={{fontSize:9,fontWeight:700,color:impClr,background:"rgba(0,0,0,0.3)",padding:"2px 6px",borderRadius:4,border:"1px solid "+impClr+"44"}}>{ev.impact}</span>
                      </div>
                    </div>
                    {ev.significance&&<div style={{fontSize:11,color:C.txt1,lineHeight:1.5}}>→ {ev.significance}</div>}
                  </div>;
                })}
              </div>}
            </div>}
            ...
            {(brief.monthlyOutlook||brief.quarterlyView)&&<div style={{background:C.bg1,border:"1px solid "+C.border,borderRadius:10,padding:"13px",marginBottom:10}}>
              <div style={{fontSize:11,color:C.txt1,letterSpacing:".1em",marginBottom:10,fontWeight:600}}>📊 MACRO OUTLOOK</div>
              {brief.monthlyOutlook&&<div style={{marginBottom:8}}>
                <div style={{fontSize:10,color:C.blue,letterSpacing:".08em",marginBottom:3}}>THIS MONTH</div>
                <div style={{fontSize:12,color:C.txt0,lineHeight:1.65}}>{brief.monthlyOutlook}</div>
              </div>}
              {brief.quarterlyView&&<div>
                <div style={{fontSize:10,color:C.vix,letterSpacing:".08em",marginBottom:3}}>THIS QUARTER</div>
                <div style={{fontSize:12,color:C.txt0,lineHeight:1.65}}>{brief.quarterlyView}</div>
              </div>}
            </div>}

            {/* Risk scenarios */}
            {brief.riskScenarios&&brief.riskScenarios.length>0&&<div style={{background:C.bg1,border:"1px solid "+C.border,borderRadius:10,padding:"13px",marginBottom:10}}>
              <div style={{fontSize:11,color:C.txt1,letterSpacing:".1em",marginBottom:8,fontWeight:600}}>⚠ RISK SCENARIOS</div>
              <div style={{display:"grid",gap:5}}>
                {brief.riskScenarios.map(function(sc,i){
                  var pClr=sc.probability>=60?C.dn:sc.probability>=35?C.amber:C.txt2;
                  var impClr=sc.impact==="HIGH"?C.dn:sc.impact==="MEDIUM"?C.amber:C.txt2;
                  return <div key={i} style={{background:C.bg2,border:"1px solid "+C.border,borderRadius:8,padding:"10px 12px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                      <span style={{fontSize:12,fontWeight:600,color:C.txt0}}>{sc.title}</span>
                      <div style={{display:"flex",alignItems:"center",gap:5}}>
                        <span style={{fontSize:10,fontWeight:700,color:pClr}}>{sc.probability}%</span>
                        <span style={{fontSize:9,color:impClr,background:"rgba(0,0,0,0.3)",padding:"2px 5px",borderRadius:3,border:"1px solid "+impClr+"44"}}>{sc.impact}</span>
                      </div>
                    </div>
                    <div style={{fontSize:11,color:C.txt1,lineHeight:1.5,marginBottom:4}}>{sc.description}</div>
                    {sc.affectedInstruments&&sc.affectedInstruments.length>0&&<div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                      {sc.affectedInstruments.map(function(sym,j){
                        return <span key={j} style={{fontSize:9,color:C.txt2,background:C.bg1,padding:"2px 6px",borderRadius:4,border:"1px solid "+C.border}}>{sym}</span>;
                      })}
                    </div>}
                  </div>;
                })}
            ...
            {/* Trader Focus */}
            {brief.traderFocus&&<div style={{background:"rgba(40,204,120,0.07)",border:"1px solid rgba(40,204,120,0.25)",borderRadius:10,padding:"13px"}}>
              <div style={{fontSize:10,color:C.up,letterSpacing:".1em",marginBottom:6,fontWeight:600}}>🎯 TRADER FOCUS — TODAY</div>
              <div style={{fontSize:13,color:C.txt0,lineHeight:1.8}}>{brief.traderFocus}</div>
            </div>}
          </div>}
        </div>}
